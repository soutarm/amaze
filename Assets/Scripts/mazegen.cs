    ...
    void MakeBlocks() {
        ...
        for (int y = 0; y <= Maze.GetUpperBound(0); y++)  {
            for (int x = 0; x <= Maze.GetUpperBound(1); x++) {
                ...
                if (y == 1 && x == 0)
                {
                    // Ensure we have an opening
                }
                else if (Maze[y, x] == 1)
                {
                    // clone our cube to add to the wall
                    Instantiate(brickPrefab, new Vector3(y * cubeSize + xOffset, 0, x * cubeSize + yOffset), Quaternion.identity);
                }
